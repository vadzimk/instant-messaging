...
async def signup_user(user: p.CreateUserSchema, session: AsyncSession = Depends(get_db)):
...
async def create_contact(contact_fields: p.CreateContactSchema,
...
async def get_contacts(user: p.GetUserSchema = Depends(get_user), session: AsyncSession = Depends(get_db)):
